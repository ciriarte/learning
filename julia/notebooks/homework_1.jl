...
student = (name = "Carlos Iriarte", kerberos_id = "ciriarte")
...
function demean(x)
	m = mean(x)
	return broadcast(-,x,m)
end

# ╔═╡ a801b380-f509-11ea-12ef-bfd4ece4156b
a_copy_of_random_vect = copy(random_vect); # in case demean modifies `x`

# ╔═╡ b0745c82-f509-11ea-3eb1-f3d0476bd373
demean(a_copy_of_random_vect)
...
function mean_colors(image)
	cs = cumsum(image, dims=1)
	(w,h) = size(cs)
	total = w * h
	return (cs[w,h].r / total, cs[w,h].g / total, cs[w,h].b / total)
end
...
The image is unrecognisable with intensity 2.73
"""
...
# ╔═╡ 83496ec6-f553-11ea-3652-f13cb820f6d2
@bind l_box Slider(1:8, show_value=true)

# ╔═╡ ca1ac5f4-ee1c-11ea-3d00-ff5268866f87
colored_line(v)

# ╔═╡ cc930c72-f553-11ea-2e48-e5b5e32952ae
...
function convolve_vector(v, k)
	v_prime = zeros(length(v))
	for i in 1:length(v_prime)
		l = (length(k) - 1) ÷ 2
		for n in -l:l
			v_prime[i] += extend(v, i + n) * k[n + l + 1]
		end
	end
	return v_prime
end
...
function gaussian_kernel(n)
	result = [exp(-x^2/2)/sqrt(2*pi) for x=-0:n]
	total = sum(result)
	return broadcast(/, result, total)
end
...
@bind gaussian_kernel_size_1D Slider(1:3, show_value=true)
# change this value, or turn me into a slider!
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	for j in 1:M_h
		for i in 1:M_w
			for k in -Kk:Kk
				for l in -Kl:Kl
					M_prime[i,j] += extend_mat(M, i + l, j + k) * K[l + Kl + 1,k + Kk + 1]
				end
			end
		end
	end
...
function with_gaussian_blur(image)
	sigma = 1
	sz = 5
	kernel = [exp(-(x^2+y^2)/2)/(2*pi*sigma^2) for x=-sz:sz, y in -sz:sz]
...
function with_sobel_edge_detect(image)
	Gx = [1;2;1]*[1 0 -1]
	Gy = [1;0;-1]*[1 2 1]
	hz = convolve_image(image, Gx)
	vt = convolve_image(image, Gy)
	
	w,h = size(image)
	
	newImage = [Gray(sqrt(broadcast(^,hz[i,j].r,2) + broadcast(^,vt[i,j].r,2))) for i=1:w, j=1:h]
	return newImage
end
